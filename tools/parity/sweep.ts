...
interface Payload {
  ...
  casefold: { unicode: string; scalars: number; changed: Record<string, string> };
}
...
let versionSkew = 0;
let foldChecked = 0;
const knownSkew = CASEFOLD_VERSION_SKEW[payload.casefold.unicode];
const skewPoints = new Set(knownSkew ?? []);
for (let cp = 0; cp < 0x110000; cp++) {
  ...
  foldChecked++;
  if (got !== expected) {
    if (skewPoints.has(cp)) {
      versionSkew++;
    } else {
      foldBad++;
    }
    if (!skewPoints.has(cp) && foldMismatches.length < 10) {
...
process.stdout.write(JSON.stringify({
  ...
  casefold: {
    checked: foldChecked,
    mismatched: foldBad,
    examples: foldMismatches,
    unicode: payload.casefold.unicode,
    tableUnicode: CASEFOLD_UNICODE,
    versionSkew,
    unknownUnicode: knownSkew === undefined ? payload.casefold.unicode : null,
  },
}, null, 2) + "\n");
