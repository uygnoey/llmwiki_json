...
class ExportMarkdownTest(WorkspaceCase):
    def test_writes_one_file_per_page_plus_manifest(self) -> None:
        self.write_demo()
        result = llmwiki.export_markdown(self.ws)
        out = self.root / "index" / "markdown"
        self.assertEqual(result["pages"], 3)
        self.assertEqual(result["out"], "index/markdown")
        self.assertEqual(sorted(p.name for p in out.glob("*.md")),
                         ["alpha.md", "beta.md", "gamma.md"])
        manifest = self.read_json("index/markdown/manifest.json")
        self.assertEqual({row["id"] for row in manifest["pages"]},
                         {"page:alpha", "page:beta", "page:gamma"})
        self.assertEqual(manifest["generated_from"], "wiki")

    def test_exported_text_matches_the_renderer_and_its_hash(self) -> None:
        self.write_demo()
        llmwiki.export_markdown(self.ws)
        manifest = self.read_json("index/markdown/manifest.json")
        for row in manifest["pages"]:
            text = (self.root / "index" / "markdown" / row["file"]).read_text(encoding="utf-8")
            page = llmwiki.find_page(self.ws, row["id"])
            self.assertEqual(text, llmwiki.render_markdown(page))
            self.assertEqual(row["sha256"], llmwiki.sha(text))

    def test_export_is_regenerated_wholesale(self) -> None:
        self.write_demo()
        llmwiki.export_markdown(self.ws)
        stale = self.root / "index" / "markdown" / "deleted-page.md"
    ...
    def test_custom_output_directory(self) -> None:
        self.write_demo()
        target = self.root / "tmp-export"
        result = llmwiki.export_markdown(self.ws, target)
        self.assertEqual(result["out"], "tmp-export")
        self.assertTrue((target / "alpha.md").exists())
